from peewee import *
from YkForm.models import BaseModel
...
class Post(BaseModel):
    ...
    def extend(cls):
        ...
        return cls.select(cls, User.id, User.nick_name,User.mobile).join(User)
...
class PostComment(BaseModel):
    ...
    def extend(cls):
        ...
        return cls.select(cls, Post, relyed_user.id, relyed_user.nick_name, author.id, author.nick_name
            ...
            Post, join_type=JOIN.LEFT_OUTER, on=cls.post).switch(cls).join(
            author, join_type=JOIN.LEFT_OUTER, on=cls.user).switch(cls).join(
